// Copper Shores Frontend - Shared application helpers
...
function updateActiveNav() {
    ...
    const currentFeature = currentPath === '/' || currentPath.startsWith('/home/')
        ? 'home'
    ...
    navLinks.forEach(link => {
        ...
        const linkFeature = linkPath === '/'
            ? 'home'
            : linkPath.split('/').filter(Boolean)[0];
        link.classList.remove('active');
        
        if (linkFeature === currentFeature) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentPath = window.location.pathname;
    if (currentPath === '/' || currentPath === '/home/' || currentPath === '/home/index.html') {
        loadHomePartyOverview();
    } else if (currentPath === '/players/' || currentPath === '/players/index.html') {
        initPlayersPage();
    } else if (currentPath === '/players/detail.html') {
        // The player detail page owns its page-specific DOMContentLoaded handler.
    }
});
...
function loadPlayersList() {
    fetch(`${API_BASE_URL}/players`)
        ...
        .then(resp => {
            ...
            players.forEach(p => {
                const tr = document.createElement('tr');
                tr.addEventListener('click', ()=>{ window.location = `/players/detail.html?id=${p.id}`; });
